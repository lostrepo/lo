...
class ResponseParser {
  ...
  #max_num_headers = ptr(new Uint32Array(1))
  ...
  constructor (rb, n_headers = 16) {
    ...
    this.#max_num_headers[0] = n_headers
    // todo - refactor this
    if (n_headers !== 16) {
      this.#raw_headers = ptr(new Uint32Array(8 * n_headers))
    }
    // this should just be size, off, we know the ptr
    this.parse = (new Function('parse', 'memcpy', `
    return function (len = ${rb.size}) {
      memcpy(${this.#num_headers.ptr}, ${this.#max_num_headers.ptr}, ${this.#max_num_headers.byteLength})
      return parse(${rb.ptr}, len, ${this.#minor_version.ptr},
        ${this.#status_code.ptr}, ${this.#message.ptr}, ${this.#message_len.ptr},
        ${this.#raw_headers.ptr}, ${this.#num_headers.ptr}, 0)
    }
    `))(pico.parse_response, lo.core.memcpy)
  }
...
class RequestParser {
  ...
  #max_num_headers = ptr(new Uint32Array(2))
  #decoder = ptr(new Uint8Array(pico.struct_phr_chunked_decoder_size))
  #chunk_size = ptr(new Uint32Array(2))
  rb = ptr(new Uint8Array(0))
