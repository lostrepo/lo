...
if (os === 'mac') {
  ...
  link_args = '-s -fno-exceptions -O3 -static-libstdc++ -static-libgcc'
  OPT = `${OPT} -march=native -mtune=native`
  cc_compiler = 'g++'
  c_compiler = 'gcc'
  const bad_warn_idx = WARN.indexOf('-Wno-error=unknown-warning-option')
  if (bad_warn_idx != -1) WARN.splice(bad_warn_idx)
  WARN.push('-Wno-comment')
}
