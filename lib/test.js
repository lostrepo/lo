import { list_children, is_file } from "lib/fs.js"
 ...
 * @template {string} T
 * @param {T} name
 * @returns {`${typeof fake_module_load}${T}${typeof fake_module_postfix}`} */
const fake_load_module_name = (name) => `${fake_module_load}${name}${fake_module_postfix}`
/**
 * @template {string} T
 * @param {T} name
 * @returns {`${typeof fake_module_library}${T}${typeof fake_module_postfix}`} */
...
/**@param {LibApi} api */
const get_lo_lib_api_strings = (api) => Object.keys(api).map((k) => {
  let res = ''
  const api_k = api[k]
  ...
  const params_strs = parameters.map((v, i) => paramWithType(`p${i}`, optional?.[i], v))
...
const get_lo_lib_constants_strings = (constants) => Object.keys(constants)
  .map(k => `  ${k}: ${get_param_type_str(constants[k])};\n`)

/**@param {string[]} structs */
const get_lo_lib_structs_strings = (structs) => structs
  .map(s => `  struct_${s}_size: number;\n`)
...
const get_lo_libs_string = (lib_exports) => {
  ...
  for (let i = 0; i < lib_exports.length; i++) {
    const def = lib_exports[i]
    const decl_string = is_fake_module_load
      ? `declare module "${fake_library_module_name(def.name)}"{ export * from '${
        fake_load_module_name(def.name)}' }
declare module "${fake_load_module_name(def.name)}"`
      : def.name
    const decl_props_strings = get_lo_lib_obj_props_strings(def)
    const decl_final_string = is_fake_module_load
      ? `${decl_string}{${decl_props_strings.map(s => `export const ${s}`).join('')}}\n`
      : `  ${decl_string}: {\n  ${decl_props_strings.join('  ')}  }`

    native_libs_str = `${native_libs_str}\n${decl_final_string}`
  }

  return is_fake_module_load ? native_libs_str : `interface NativeLibs {\n${native_libs_str}\n}`
}
...
const paramWithType = (paramName, opt, paramType) =>
  /**@type {const}*/(`${paramName}${opt ? '?' : ''}: ${get_param_type_str(paramType)}`)

export const test = async () => {
  taggedTypeMap.clear()
  is_fake_module_load = false
  const apis = get_apis()
...
${is_fake_module_load ? get_native_libs_string(lib_exports): ''}
${get_lo_libs_string(lib_exports)}
// Native tagged types
${Array.from(taggedTypeMap.values()).join('\n')}
${is_fake_module_load ? `declare module "${fake_api_js_module_name}" {
