...
// !!! need similar func on lo.core.loader/lo.core.loader_sync for "lo:*" js imports to work !!!
export const fake_module_lo_core_loader = (/** @type {string} */ specifier) => {
  // internal fake modules
  if (/^lo\:/.test(specifier)) {
    let export_str = ''
    // clear cache on load (to keep props fresh for dynamic import/require)
    // let clear_cache_str = `lo.module_cache.delete('${specifier}')`
    let clear_cache_str = ``

    const [_, core_or_method, native_lib_name] = specifier.split(':')
    if (!core_or_method)
      export_str = `export const { ${Object.keys(lo)} } = lo`
    else if (core_or_method == 'core')
      export_str = `export const { ${Object.keys(lo.core)} } = lo.core`
    else if (core_or_method == 'api.js')
      export_str = `export const {} = {}`
    else if (!native_lib_name)
      export_str = `throw 'unknown specifier "${specifier}"'`
    else if (core_or_method == 'load' || core_or_method == 'library')
      export_str = `export const { ${
        Object.keys(lo[core_or_method](native_lib_name)[native_lib_name])
      } } = lo['${core_or_method}']('${native_lib_name}').${native_lib_name}`
    else
      export_str = `throw 'unknown method "${core_or_method}" for specifier "${specifier}"'`

    return `${clear_cache_str}\n${export_str}`
  }
...
const fake_module_prefix = 'lo:'
const fake_module_postfix = ':'
const fake_module_lo = fake_module_prefix
const fake_module_core = `${fake_module_prefix}core${fake_module_postfix}`
const fake_module_load = `${fake_module_prefix}load${fake_module_postfix}`
const fake_module_library = `${fake_module_prefix}library${fake_module_postfix}`
const fake_api_js_module_name = `${fake_module_prefix}${api_file_name}${fake_module_postfix}`
const fake_load_module_name = (name) => `${fake_module_load}${name}${fake_module_postfix}`
const fake_library_module_name = (name) => `${fake_module_library}${name}${fake_module_postfix}`
const fake_module_native_core = fake_load_module_name('core')
...
${Object.keys(api).map((k) => {
      ...
      res = `${jsdoc_str}${man_str}  export function ${k}(${parameters_strings}): ${ret_str};\n`
      return res;
    }).join('')}${Object.keys(constants).map(k => `  export var ${k}: number;\n`).join('')}${
      structs.map(s => `  export var ${s}: number;\n`).join('')}}`
    native_libs_str = `${native_libs_str}
declare module "${fake_library_module_name(def.name)}" { export * from '${fake_load_module_name(def.name)}' }
declare module "${fake_load_module_name(def.name)}" ${exports_obj}
`
  }

  return native_libs_str
}
...
${get_lo_libs_string(lib_exports)}
declare module "${fake_api_js_module_name}" {
...
type Core = typeof import('${fake_module_core}');
declare module '${fake_module_core}' {
  export * from '${fake_module_native_core}';
  export function dlsym(handle: number, name: string): number;
  export function dlopen(path: string, flags: number): number;
  ...
  export var os: OS;
  export var arch: ARCH;
  export var engine: ENGINE;
  export var little_endian: boolean;
  export var homedir: string;
  export var defaultWriteFlags: number;
  export var defaultWriteMode: number;
  ...
  export function getcwd(ptr: number, num: number, buf: Uint32Array): void;
  export function getenv(name: string, buf: Uint32Array): void;
  export function write_string(num: number, str: string): number;
  export function readFile(path: string, flags?: number, size?: number): Uint8Array;
  ...
  export var loader: void | ((specifier: string, resource: string) => string);
  export var sync_loader: void | ((specifier: string, resource: string) => string);
  export var binding_loader: void | (<T extends string>(name: T) => Library<T>);
}`

const runtime_type = () => `// TODO: autogenerate
type Runtime = typeof import('${fake_module_lo}');
declare module "${fake_module_lo}" {
  // validate with list from: lo eval 'console.log(\`"\${Object.getOwnPropertyNames(lo).join(\`":unknown;"\`)}":unknown;\`)'
  export var moduleCache: Map<string, ReturnType<Runtime['loadModule']>>;
  export var libCache: Map<string, object>;
  export var requireCache: Map<string, object>;
  export var start: number;
  export var errno: number;
  export var colors: Record<Uppercase<string>, string>;
  export var core: Core;
  export function libraries(): string[];
  export function builtins(): string[];
  export function assert(expression: any, message?: string | Function): any;
  export function cstr(str: string): Ptr<Uint8Array>;
  export function load<T extends NativeLibsKeys>(name: T): NativeLibXExport<T>;
  export function library<T extends string | number>(name: T): Library<T>;
  /**
  ...
  export function getAddress(buf: TypedArray): number;
  export function utf8Length(str: string): number;
  export function utf8EncodeInto(str: string, buf: TypedArray): number;
  export function utf8EncodeIntoAtOffset(str: string, buf: TypedArray, off: number): number;
  export function utf8_decode(address: number, len?: number): string;
  export function latin1Decode(address: number, len?: number): string;
  export function utf8Encode(str: string): Uint8Array;
  export var utf8Decode: Runtime['utf8_decode'];
  ...
  export function addr(handle: TypedArray): number;
  export var version: RuntimeVersion;
  export var args: string[];
  export var argv: number;
  export var argc: number;
  export var workerSource: string;
  export function builtin(path: string): string;
  export function os(): OS;
  export function arch(): ARCH;
  export function getenv(str: string): string;
  export function evaluateModule<T extends object>(identifier: number): Promise<T>;
